...
public class WebSecurityConfig {
    ...
    public UserDetailsService userDetailsService(){
        ...
        var william = User.withUsername("william")
                .password("william")
                .roles(DEPARTMENT_CREATE,DEPARTMENT_READ,DEPARTMENT_PAG_VIEW).build();
        var lucas = User.withUsername("lucas").password("lucas")
                .roles(CUSTOMERS_READ,CUSTOMERS_PAG_VIEW).build();
        var tom = User.withUsername("tom").password("tom").roles()
                .build();
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Throwable{
        ...
        http.authorizeRequests().expressionHandler(expressionHandler())
                .requestMatchers("/","home","/bootstrap/**").permitAll()
